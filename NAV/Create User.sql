...
USE [BCW_DEV]  -- NAV database
GO

DECLARE @UserID varchar(100)
SET @UserID = 'Keef-Laptop\Keith'  -- Windows Login you want to add
...
DECLARE @j AS int
DECLARE @Grp AS int
SELECT @StringSID = 'S-' + CONVERT(VARCHAR, CONVERT(INT, CONVERT(VARBINARY, SUBSTRING(@BinarySID, 1, 1)))) 
SELECT @StringSID = @StringSID + '-' + CONVERT(VARCHAR, CONVERT(INT, CONVERT(VARBINARY, SUBSTRING(@BinarySID, 3, 6))))
SET @j = 9
SET @i = LEN(@BinarySID)
SET @Grp = 1
...
ELSE BEGIN
  ...
  VALUES(@UserGUID         ,@UserID    ,''         ,0      ,'1/1/1753'   ,@StringSID           ,0                ,0             ,''                    ,''             ,''                   ,@UserGUID)  
...
ELSE BEGIN  
  ...
  VALUES(@UserGUID         ,'SUPER'  ,''            ,''     ,@UserGUID) 
...
ELSE BEGIN
  INSERT INTO [User Property]
        ([User Security ID],[Password],[Name Identifier],[Authentication Key],[WebServices Key],[WebServices Key Expiry Date],[Authentication Object ID],[Directory Role ID],[Directory Role ID List],[Telemetry User ID])
  VALUES(@UserGUID         ,''        ,''               ,''                  ,''               ,'1/1/1753'                   ,0                         ,'','',@UserGUID)
